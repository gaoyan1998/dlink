...
ADD settings.xml ~/.m2/settings-docker.xml
ADD settings.xml /user/share/maven/conf/settings.xml
RUN mvn install -Dmaven.test.skip=true -P prod,scala-2.12,flink-single-version,1.18,jdk11,fast && \
    mvn package -Dmaven.test.skip=true -P prod,scala-2.12,flink-single-version,1.18,jdk11,fast && \
    cd build && \
